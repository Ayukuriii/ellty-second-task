...
| Field | Type | Description |
|-------|------|-------------|
| `token` | `string` | JWT for `Authorization: Bearer <token>` on protected routes (not yet used by public auth endpoints). |
...
No authenticated auth endpoint is exposed in this scope; middleware is reserved for later routes (for example node creation).
...
- `GET /api/nodes`, node subtree, replies
